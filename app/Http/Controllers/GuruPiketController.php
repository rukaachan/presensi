    ...
    public function detailProfil(Request $request, Guru $guru)
    {
        ...
        if ($guruRecord === null) {
            return view('layout.profile-unavailable', [
                'message' => 'Akun guru piket ini belum terhubung dengan data guru.',
                'backUrl' => route('guru-piket.dashboard'),
            ]);
        }
    ...
    public function detailPresensi(Request $request, PresensiSiswa $presensi)
    {
        $data = [
            'presensi' => $presensi
                ...
                ->where('presensi_siswa.id_presensi', $request->id)->first(),
        ];
    ...
    public function updatePresensi(Request $request, Role $role, PresensiSiswa $presensi)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($user, $role, $legacy, $data, $evidence): void {
                $roleLabel = $role->where('id_role', $user->id_role)->value('nama_role') ?: 'Guru Piket';
                $legacy->update([
