...
@section('judul', 'Catat presensi kelas')
@section('page-description', 'Ambil foto untuk mengirimkan catatan kehadiran kelas hari ini.')
...
@section('isi')
    @if (! $siswa?->id_siswa)
        <section class="operations-empty-state account-empty-state" aria-labelledby="capture-unavailable-title">
            ...
            <strong id="capture-unavailable-title">Profil pengurus belum siap</strong>
            <p>Akun ini belum terhubung dengan data siswa. Hubungi Tata Usaha sebelum mengelola presensi.</p>
        </section>
    @else
    <section class="capture-page" aria-labelledby="capture-title">
        <header class="capture-heading">
            <div>
                <p class="eyebrow">Presensi kelas</p>
                <h2 id="capture-title">Catat kehadiran dengan tertib.</h2>
        ...
        <form method="POST" action="{{ route('pengurus-kelas.webcam.capture') }}" id="presensiForm" class="capture-form">
            @csrf
...
@section('footer')
    ...
    <script>
        ...
        async function takeSnapshotWithCheck() {
            ...
            try {
                const response = await fetch('{{ route('pengurus-kelas.webcam.check_snapshot') }}', {
                    method: 'POST',
