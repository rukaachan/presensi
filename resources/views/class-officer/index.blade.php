...
@section('title', __('pages.class_officer_dashboard'))
@section('content')
<section class="dashboard-intro" aria-labelledby="dashboard-title"><div><p class="eyebrow">{{ __('pages.class_summary') }}</p><h2 id="dashboard-title">{{ __('pages.class_officer_headline') }}</h2><p>{{ __('pages.class_officer_description') }}</p></div><a class="quick-action rounded-md bg-accent text-accent-foreground ring-1 ring-accent" href="{{ route('class-officer.attendance.create') }}"><span>{{ __('nav.record_attendance') }}</span><i class="ph-bold ph-arrow-right" aria-hidden="true"></i></a></section>
...
@include('layout.partials.metric-bars',['metrics'=>$metrics,'visualTitle'=>__('pages.class_status'),'visualDescription'=>__('pages.class_status_description')])
@endsection
