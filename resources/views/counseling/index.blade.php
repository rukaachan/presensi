...
@section('title', __('pages.counseling_dashboard'))
@section('content')
<section class="dashboard-intro" aria-labelledby="dashboard-title"><div><p class="eyebrow">{{ __('pages.attendance_operations') }}</p><h2 id="dashboard-title">{{ __('pages.counseling_headline') }}</h2><p>{{ __('pages.counseling_description') }}</p></div><a class="quick-action rounded-md bg-accent text-accent-foreground ring-1 ring-accent" href="{{ route('counseling.attendance.index') }}"><span>{{ __('nav.review_attendance') }}</span><i class="ph-bold ph-arrow-right" aria-hidden="true"></i></a></section>
